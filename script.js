...
const categories = {
  ...
  // ── カレー・シチュー ──────────────────────────────────────
  curry_stew:    { label:"カレー・シチュー・丼", emoji:"🍛", color:"#C8A040", servingG:400,
    keywords:["カレー","シチュー","ビーフシチュー","クリームシチュー","グラタン","ドリア","カレーライス"],
    perServing:{cal:580,prot:18,fat:16,carb:88,fib:4,sod:1200,cal_m:50,iron:2.5,vitC:8,B1:0.2,B2:0.15,B6:0.3}},
  donburi:       { label:"どんぶりもの",         emoji:"🥣", color:"#C88840", servingG:350,
    keywords:["親子丼","牛丼","豚丼","海鮮丼","天丼","カツ丼","うな丼","丼","どんぶり","煮込み丼","麻婆丼","チャーシュー丼","マグロ丼","サーモン丼","鉄火丼","中華丼","他人丼"],
    perServing:{cal:580,prot:20,fat:12,carb:88,fib:2,sod:1400,cal_m:40,iron:2,vitC:5,B1:0.2,B2:0.2,B6:0.3}},
  // ── 健康・機能性食品 ──────────────────────────────────────
  low_carb:      { label:"糖質カット食品",       emoji:"💚", color:"#60A870",
    keywords:["糖質ゼロ","糖質オフ","糖質カット","低糖質","こんにゃく麺","ゼロ麺","豆腐麺","糖質制限","ロカボ","カーボオフ","ブランパン","低糖パン","こんにゃく","しらたき"],
    perServing:{cal:70,prot:3,fat:0.5,carb:8,fib:3.5,sod:200,cal_m:20,iron:0.5,vitC:0,B1:0.04,B2:0.04,B6:0.04}},
  low_fat:       { label:"低脂肪・無脂肪食品",   emoji:"🥤", color:"#90C0D0",
    keywords:["低脂肪","無脂肪","ノンファット","低脂肪牛乳","スキムミルク","無脂肪ヨーグルト","脱脂粉乳","低脂肪チーズ","低カロリー乳","ライト乳"],
    perServing:{cal:60,prot:5.5,fat:0.5,carb:8,fib:0,sod:80,cal_m:180,iron:0.1,vitC:1,B1:0.03,B2:0.15,B6:0.04}},
  soy_meat:      { label:"大豆ミート・代替肉",   emoji:"🌱", color:"#70A860",
    keywords:["大豆ミート","代替肉","植物性肉","豆腐ハンバーグ","ソイミート","ベジミート","畑の肉","フェイクミート","ビーガン肉","植物性タンパク"],
    perServing:{cal:160,prot:18,fat:5,carb:10,fib:4,sod:400,cal_m:80,iron:3,vitC:0,B1:0.15,B2:0.1,B6:0.2}},
  zero_drink:    { label:"ゼロカロリー飲料",     emoji:"🫧", color:"#A8C8E0", servingMl:350,
    keywords:["ゼロカロリー","カロリーゼロ","カロリーオフ","ダイエットコーラ","ゼロシュガー","糖類ゼロ飲料","ゼロ飲料","ゼロサイダー","カロリーフリー"],
    perServing:{cal:3,prot:0,fat:0,carb:0.5,fib:0,sod:20,cal_m:0,iron:0,vitC:0,B1:0,B2:0,B6:0}},
  health_bar:    { label:"機能性食品・栄養補助",   emoji:"🍫", color:"#8878A0",
    keywords:["栄養補助食品","カロリーメイト","バランスパワー","ウィダー","SOYJOy","プロテインバー","機能性食品","栄養バー","グラノーラバー"],
    perServing:{cal:180,prot:10,fat:7,carb:22,fib:3,sod:120,cal_m:100,iron:2,vitC:10,B1:0.25,B2:0.25,B6:0.25}},
  // ── ファストフード ──────────────────────────────────────
...
let histViewMode = "7day";   // "7day" | "month"
let histYM = { year: new Date().getFullYear(), month: new Date().getMonth()+1 };
...
function extractQuantity(text) {
  let count = 1, mlOverride = null, gramOverride = null;
  // gram (must come before ml to avoid "gram" conflicts)
  const gramM = text.match(/(\d+(?:\.\d+)?)\s*(?:g|ｇ|グラム)(?!l|L)/i);
  if(gramM) gramOverride = parseFloat(gramM[1]);
  // ml
  const mlM = text.match(/(\d+(?:\.\d+)?)\s*(?:ml|mL|ミリ|cc)/i);
  if(mlM) mlOverride = parseFloat(mlM[1]);
  // count (杯 also used for donburi/rice)
  const cntM = text.match(/(\d+(?:\.\d+)?)\s*(?:個|枚|切れ|本|杯|膳|缶|パック|食|人前|皿|袋|合|グラス|ボトル|串|つ|ピース|slice|piece|cup)/);
  if(cntM) count = parseFloat(cntM[1]);
  // bare trailing number
  if(!gramM && !mlM && !cntM) { const nm=text.match(/\s(\d+)\s*$/); if(nm) count=parseFloat(nm[1]); }
  return { count, mlOverride, gramOverride };
}

function stripQuantity(text) {
  return text
    .replace(/\d+(?:\.\d+)?\s*(?:ml|mL|ミリ|cc)/gi,"")
    .replace(/\d+(?:\.\d+)?\s*(?:g|ｇ|グラム)(?!l|L)/gi,"")
    .replace(/\d+(?:\.\d+)?\s*(?:個|枚|切れ|本|杯|膳|缶|パック|食|人前|皿|袋|合|グラス|ボトル|串|つ|ピース)/g,"")
    .replace(/\s+\d+\s*$/,"")
    .trim();
}
...
function parseInput(text) {
  ...
  tokens.forEach(token => {
    const { count, mlOverride, gramOverride } = extractQuantity(token);
    const found = matchCategory(token);
    if(found) {
      const { catKey, cat } = found;
      let scale = count;
      if(mlOverride && cat.servingMl) {
        scale = mlOverride / cat.servingMl;              // 500ml → scale by ml
      } else if(gramOverride && cat.servingG) {
        scale = gramOverride / cat.servingG;              // 350g → scale by g
      } else if(gramOverride && !cat.servingG && !cat.servingMl) {
        // No servingG defined: treat 100g as 1 serving approximation
        scale = gramOverride / 100;
      }
...
(function init() {
  ...
  // Show setup only on first visit
  ...
  // ── History view toggle ──────────────────────────────
  $("hvt-week").addEventListener("click", () => {
    histViewMode = "7day";
    $("hvt-week").classList.add("active"); $("hvt-month").classList.remove("active");
    hide("month-nav"); renderMealHistory();
  });
  $("hvt-month").addEventListener("click", () => {
    histViewMode = "month";
    $("hvt-month").classList.add("active"); $("hvt-week").classList.remove("active");
    show("month-nav"); updateMonthLabel(); renderMealHistory();
  });
  $("mn-prev").addEventListener("click", () => {
    histYM.month--; if(histYM.month<1){ histYM.month=12; histYM.year--; }
    updateMonthLabel(); renderMealHistory();
  });
  $("mn-next").addEventListener("click", () => {
    const now=new Date();
    if(histYM.year===now.getFullYear()&&histYM.month===now.getMonth()+1) return;
    histYM.month++; if(histYM.month>12){ histYM.month=1; histYM.year++; }
    updateMonthLabel(); renderMealHistory();
  });
...
function renderFinalSummary(ref) {
  const el = $("final-summary-body");
  const cutoff = new Date(); cutoff.setDate(cutoff.getDate()-30);
  const recent = records.filter(r=>r.date>=toDS(cutoff));

  if(recent.length < 3) {
    el.innerHTML = `<div class="fs-nodata">📋 3食以上記録すると総合分析が表示されます<br><span style="font-size:.75rem;color:var(--text3)">現在 ${recent.length} 食の記録</span></div>`;
    return;
  }

  // Build per-day averages over last 30 days
  const byDate = {};
  recent.forEach(r=>{ if(!byDate[r.date]) byDate[r.date]=[]; byDate[r.date].push(r); });
  const dates = Object.keys(byDate);
  const nutKeys = ["calories","protein","fat","carbs","fiber","sodium","calcium","iron","vitaminC","B1","B2","B6"];
  const LMAP = {calories:"カロリー",protein:"タンパク質",fat:"脂質",carbs:"炭水化物",fiber:"食物繊維",sodium:"塩分",calcium:"カルシウム",iron:"鉄分",vitaminC:"ビタミンC",B1:"B1",B2:"B2",B6:"B6"};
  const IMAP = {calories:"🔥",protein:"💪",fat:"🫒",carbs:"🌾",fiber:"🥦",sodium:"🧂",calcium:"🦴",iron:"⚙️",vitaminC:"🍊",B1:"🌿",B2:"🌱",B6:"🫛"};

  // Average daily achievement %
  const avgPct = {};
  nutKeys.forEach(k=>{
    const sum = dates.reduce((s,ds)=>{
      const n=sumNuts(byDate[ds]); return s+(n[k]||0)/ref[k]*100;
    },0);
    avgPct[k] = Math.round(sum/dates.length);
  });

  // Score: average of all non-sodium nutrients, capped at 100, sodium inverted
  const scoreKeys = nutKeys.filter(k=>k!=="sodium");
  const rawScore = scoreKeys.reduce((s,k)=>s+Math.min(avgPct[k],100),0) / scoreKeys.length;
  // Sodium penalty
  const sodPenalty = avgPct.sodium>100 ? Math.min((avgPct.sodium-100)*0.3, 15) : 0;
  const score = Math.round(Math.max(0, rawScore - sodPenalty));

  // Grade
  const grade = score>=85?"とても良好 🌟":score>=70?"まあまあ良好 👍":score>=55?"改善の余地あり ⚠️":"要注意 🔴";
  const gradeDesc = score>=85?"栄養バランスが全体的に安定しています。":score>=70?"多くの栄養素が目安に近いレベルです。":score>=55?"一部の栄養素に慢性的な不足があります。":"複数の栄養素が大きく不足しています。";

  // Strong points (>= 75%)
  const strong = scoreKeys.filter(k=>avgPct[k]>=75).sort((a,b)=>avgPct[b]-avgPct[a]).slice(0,3);
  // Weak points (< 60%)
  const weak   = scoreKeys.filter(k=>avgPct[k]<60).sort((a,b)=>avgPct[a]-avgPct[b]).slice(0,4);
  // Excess (>130%)
  const excess = nutKeys.filter(k=>avgPct[k]>130);

  // One concrete tip based on worst deficit
  const worst = weak[0];
  const TIPS = {
    protein:"毎食に卵・鶏肉・魚・豆腐などを1品加えるだけで大きく改善できます。",
    fat:"ナッツ・アボカド・青魚を間食や副菜に取り入れましょう。",
    carbs:"主食（ご飯・パン・麺）を毎食しっかり食べるよう意識しましょう。",
    fiber:"野菜を毎食手のひら一杯分、きのこ・海藻を1品加えましょう。",
    calcium:"毎日1杯の牛乳またはヨーグルトを習慣にすると効果的です。",
    iron:"週2〜3回は赤身肉・レバー・ほうれん草を取り入れましょう。",
    vitaminC:"ブロッコリー・パプリカ・みかんを1日1品加えると補いやすいです。",
    B1:"豚肉や大豆製品を週数回意識して食べましょう。",
    B2:"乳製品・卵を毎日摂ることで補いやすくなります。",
    B6:"肉・魚・バナナをバランスよく取り入れましょう。",
  };
  const tipText = worst ? TIPS[worst] || "" : "今の食習慣をそのまま続けましょう！バランスが良好です。";

  // Calories context
  const calAvg = Math.round(dates.reduce((s,ds)=>s+sumNuts(byDate[ds]).calories,0)/dates.length);
  const calDiff = calAvg - ref.calories;
  const calNote = Math.abs(calDiff)<100 ? "目安に近い摂取量です" : calDiff>0 ? `目安より約${Math.abs(calDiff)}kcal多め` : `目安より約${Math.abs(calDiff)}kcal少なめ`;

  // Render
  const circ=226, scoreOff=circ*(1-score/100);
  el.innerHTML = `
    <div class="fs-score-wrap">
      <div class="fs-score-ring">
        <svg viewBox="0 0 72 72" width="72" height="72">
          <circle class="fsr-bg"   cx="36" cy="36" r="30"/>
          <circle class="fsr-fill" cx="36" cy="36" r="30"
            stroke-dasharray="${circ}" stroke-dashoffset="${circ}"
            id="fsr-fill-anim"/>
        </svg>
        <div class="fs-score-num"><span class="fs-num">${score}</span><span class="fs-pt">/ 100</span></div>
      </div>
      <div class="fs-score-r">
        <div class="fs-grade">${grade}</div>
        <div class="fs-grade-sub">${gradeDesc}<br>直近${dates.length}日・${recent.length}食の記録<br>平均 ${calAvg.toLocaleString()} kcal/日（${calNote}）</div>
      </div>
    </div>
    <div class="fs-blocks">
      ${strong.length>0?`<div class="fs-block fs-block-ok">
        <div class="fs-block-hd"><span class="fs-block-ico">✅</span>よく摂れている栄養素</div>
        <div class="fs-items">${strong.map(k=>`<div class="fs-item">${IMAP[k]} ${LMAP[k]}（平均 ${avgPct[k]}%）</div>`).join("")}</div>
      </div>`:""}
      ${weak.length>0?`<div class="fs-block fs-block-bad">
        <div class="fs-block-hd"><span class="fs-block-ico">⚠️</span>慢性的に不足している栄養素</div>
        <div class="fs-items">${weak.map(k=>`<div class="fs-item">${IMAP[k]} ${LMAP[k]}（平均 ${avgPct[k]}%）</div>`).join("")}</div>
      </div>`:""}
      ${excess.length>0?`<div class="fs-block fs-block-warn">
        <div class="fs-block-hd"><span class="fs-block-ico">📈</span>摂り過ぎに注意</div>
        <div class="fs-items">${excess.map(k=>`<div class="fs-item">${IMAP[k]} ${LMAP[k]}（平均 ${avgPct[k]}%）</div>`).join("")}</div>
      </div>`:""}
      <div class="fs-block fs-block-tip">
        <div class="fs-block-hd"><span class="fs-block-ico">💡</span>今日からできるアクション</div>
        <div class="fs-tip">${tipText}</div>
      </div>
    </div>`;

  // Animate ring
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    const fill=$("fsr-fill-anim");
    if(fill) fill.style.strokeDashoffset=scoreOff;
  }));
}
...
// ═══════════════════════════════════════════════════════════
// HISTORY — helper: build byDate map
// ═══════════════════════════════════════════════════════════
...
function updateMonthLabel() {
  $("mn-label").textContent = `${histYM.year}年${histYM.month}月`;
  const now = new Date();
  $("mn-next").disabled = (histYM.year===now.getFullYear() && histYM.month===now.getMonth()+1);
}

// ─── Shared: build expanded day detail HTML ───────────────
// Shows meal list + nutrient mini-bars + deficit tags
function buildDayDetail(recs, ref) {
  // ── meal rows ──
  ...
  if (recs.length === 0) return `<div class="mh-day-meals">${mealsHtml}</div>`;

  // ── nutrient totals for the day ──
  const dn = sumNuts(recs);

  // ── mini nutrient bars ──
  const barKeys = [
    {k:"calories",ico:"🔥",lbl:"カロリー"},
    {k:"protein", ico:"💪",lbl:"タンパク質"},
    {k:"fat",     ico:"🫒",lbl:"脂質"},
    {k:"carbs",   ico:"🌾",lbl:"炭水化物"},
    {k:"fiber",   ico:"🥦",lbl:"食物繊維"},
    {k:"calcium", ico:"🦴",lbl:"Ca"},
    {k:"vitaminC",ico:"🍊",lbl:"VitC"},
    {k:"B1",      ico:"🌿",lbl:"B1"},
    {k:"B2",      ico:"🌱",lbl:"B2"},
    {k:"B6",      ico:"🫛",lbl:"B6"},
  ];
  const barsHtml = barKeys.map(({k,ico,lbl}) => {
    const v=dn[k]||0, tgt=ref[k]||1;
    const pct=Math.min(Math.round(v/tgt*100),130);
    const barW=Math.min(pct,100);
    const [fillClr,pctCls] = pct<40?["#D95F3B","pct-s-bad"]:pct>125?["#C48B2A","pct-s-warn"]:["#6B9E78","pct-s-ok"];
    return `<div class="dnb-row">
      <span class="dnb-ico">${ico}</span>
      <span class="dnb-lbl">${lbl}</span>
      <div class="dnb-track"><div class="dnb-fill" style="width:${barW}%;background:${fillClr}"></div></div>
      <span class="dnb-pct ${pctCls}">${pct}%</span>
    </div>`;
  }).join("");

  // ── deficit / excess tags ──
  const minK = ["protein","fat","carbs","fiber","calcium","iron","vitaminC","B1","B2","B6"];
  const LMAP = {protein:"タンパク",fat:"脂質",carbs:"炭水化",fiber:"食物繊維",
                calcium:"Ca",iron:"鉄分",vitaminC:"VitC",B1:"B1",B2:"B2",B6:"B6"};
  const IMAP = {protein:"💪",fat:"🫒",carbs:"🌾",fiber:"🥦",
                calcium:"🦴",iron:"⚙️",vitaminC:"🍊",B1:"🌿",B2:"🌱",B6:"🫛"};
  const deficits = minK.filter(k=>(dn[k]||0)/ref[k]*100<50);
  const sodOver  = (dn.sodium||0)>ref.sodium;
  const calOver  = dn.calories/ref.calories>1.2;

  let defTags = deficits.map(k=>`<span class="dd-tag dd-bad">${IMAP[k]} ${LMAP[k]}不足</span>`).join("");
  if(sodOver) defTags += `<span class="dd-tag dd-over">🧂 塩分過多</span>`;
  if(calOver) defTags += `<span class="dd-tag dd-over">🔥 カロリー過多</span>`;
  if(!defTags) defTags = `<span class="dd-tag dd-ok">✅ この日のバランスは良好</span>`;

  return `
    <div class="mh-day-meals">${mealsHtml}</div>
    <div class="day-nut-section">
      <div class="dns-title">栄養素（1日合計 vs 目安）</div>
      <div class="dns-grid">${barsHtml}</div>
    </div>
    <div class="day-def-row">${defTags}</div>`;
}

// ─── Dispatcher ───────────────────────────────────────────
function renderMealHistory() {
  if(histViewMode === "month") renderMonthView();
  else renderWeekView();
}
...
function renderWeekView() {
  ...
  list.innerHTML = days.reverse().map(ds => {
    const recs = (byDate[ds]||[]).sort((a,b)=>a.timestamp-b.timestamp);
...
function renderMonthView() {
  ...
  const monthDates = Object.keys(byDate).filter(d=>d.startsWith(monthPfx));
  const recordedDays = monthDates.length;
  const monthAllRecs = monthDates.flatMap(d=>byDate[d]);
  const monthN = sumNuts(monthAllRecs);

  // Per-day averages
  const avgN = {};
  const nutKeys = ["calories","protein","fat","carbs","fiber","sodium","calcium","iron","vitaminC","B1","B2","B6"];
  nutKeys.forEach(k => avgN[k] = recordedDays>0 ? monthN[k]/recordedDays : 0);

  // ── Summary card ──
  const avgCal   = recordedDays>0 ? Math.round(monthN.calories/recordedDays) : 0;
  const totalCal = Math.round(monthN.calories);

  const sumNutRows = [
    {k:"calories",ico:"🔥",lbl:"カロリー",u:"kcal"},
    {k:"protein", ico:"💪",lbl:"タンパク質",u:"g"},
    {k:"fat",     ico:"🫒",lbl:"脂質",u:"g"},
    {k:"carbs",   ico:"🌾",lbl:"炭水化物",u:"g"},
    {k:"fiber",   ico:"🥦",lbl:"食物繊維",u:"g"},
    {k:"calcium", ico:"🦴",lbl:"Ca",u:"mg"},
    {k:"iron",    ico:"⚙️",lbl:"鉄分",u:"mg"},
    {k:"vitaminC",ico:"🍊",lbl:"VitC",u:"mg"},
    {k:"B1",      ico:"🌿",lbl:"B1",u:"mg"},
    {k:"B2",      ico:"🌱",lbl:"B2",u:"mg"},
    {k:"B6",      ico:"🫛",lbl:"B6",u:"mg"},
  ];

  const nutBarsHtml = sumNutRows.map(({k,ico,lbl}) => {
    const v=avgN[k]||0, tgt=ref[k]||1;
    const pct=Math.min(Math.round(v/tgt*100),130);
    const barW=Math.min(pct,100);
    const [fillClr,pctCls]=pct<40?["#E88A70","pct-bad-w"]:pct>125?["#DAAC60","pct-warn-w"]:["#8BC89E","pct-ok-w"];
    return `<div class="ms-bar-row">
      <span class="ms-bar-lbl">${ico} ${lbl}</span>
      <div class="ms-bar-wrap"><div class="ms-bar-fill" style="width:${barW}%;background:${fillClr}"></div></div>
      <span class="ms-bar-pct ${pctCls}">${pct}%</span>
    </div>`;
  }).join("");

  // Month chronic deficits (avg < 75%)
  const minK = ["protein","fat","carbs","fiber","calcium","iron","vitaminC","B1","B2","B6"];
  const IMAP2={protein:"💪",fat:"🫒",carbs:"🌾",fiber:"🥦",calcium:"🦴",iron:"⚙️",vitaminC:"🍊",B1:"🌿",B2:"🌱",B6:"🫛"};
  const LMAP2={protein:"タンパク質",fat:"脂質",carbs:"炭水化物",fiber:"食物繊維",calcium:"Ca",iron:"鉄分",vitaminC:"VitC",B1:"B1",B2:"B2",B6:"B6"};
  const monthDeficits = recordedDays>=3
    ? minK.filter(k=>(avgN[k]||0)/ref[k]*100<75).sort((a,b)=>(avgN[a]||0)/ref[a]-(avgN[b]||0)/ref[b])
    : [];

  const defTagsHtml = monthDeficits.length>0
    ? monthDeficits.slice(0,6).map(k=>`<span class="ms-def-tag">${IMAP2[k]} ${LMAP2[k]} ${Math.round((avgN[k]||0)/ref[k]*100)}%</span>`).join("")
    : (recordedDays>=3 ? `<div class="ms-good-msg">✅ 今月の栄養バランスは概ね良好です！</div>` : `<div class="ms-good-msg">3日以上記録すると月の傾向を分析できます</div>`);

  let html = `<div class="month-summary">
    <div class="ms-stats">
      <div class="ms-item"><div class="ms-num">${recordedDays}<span style="font-size:.75rem;opacity:.5">/${daysInMonth}</span></div><div class="ms-lbl">記録日数</div></div>
      <div class="ms-item"><div class="ms-num">${avgCal.toLocaleString()}</div><div class="ms-lbl">平均 kcal/日</div></div>
      <div class="ms-item"><div class="ms-num">${(totalCal/1000).toFixed(1)}<span style="font-size:.75rem;opacity:.5">万</span></div><div class="ms-lbl">月合計 kcal</div></div>
    </div>
    ${recordedDays>0 ? `<div class="ms-nut-title">1日平均 栄養素達成率</div><div class="ms-nut-bars">${nutBarsHtml}</div>` : ""}
    ${monthDeficits.length>0?`<div class="ms-nut-title" style="margin-top:.5rem">今月の不足栄養素</div>`:""}<div class="ms-def-row">${defTagsHtml}</div>
  </div>`;

  // ── Day list (all days in month, newest first) ──
  for(let d=daysInMonth; d>=1; d--) {
    const ds = `${year}-${String(month).padStart(2,"0")}-${String(d).padStart(2,"0")}`;
    if(ds > today) continue; // skip future
    const recs = (byDate[ds]||[]).sort((a,b)=>a.timestamp-b.timestamp);
    const W    = ["日","月","火","水","木","金","土"];
    const dow  = new Date(ds+"T00:00:00");
    const wdL  = W[dow.getDay()];
    const dayLbl = `${month}月${d}日（${wdL}）${ds===today?" 🌟":""}`;
    const totalC = recs.reduce((s,r)=>s+(r.calories||0),0);
    const hasRec = recs.length>0;
    html += `<div class="mh-day" id="mhd-${ds}">
      <div class="mh-day-hd" onclick="toggleDay('${ds}')">
        <span class="mh-day-date" style="${!hasRec?"opacity:.45":""}">${dayLbl}</span>
        <span class="mh-day-cal" style="${!hasRec?"opacity:.38":""}">${hasRec?totalC.toLocaleString()+" kcal":"未記録"}</span>
        <span class="mh-day-toggle" style="${!hasRec?"opacity:.2":""}">▾</span>
      </div>
      ${hasRec ? buildDayDetail(recs, ref) : ""}
    </div>`;
  }
  ...
  // Animate bars
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    list.querySelectorAll(".ms-bar-fill").forEach(el=>{ const w=el.style.width; el.style.width="0%"; requestAnimationFrame(()=>{ el.style.width=w; }); });
  }));

  // Auto-open today
  const todayEl = $(`mhd-${today}`);
  if(todayEl && histYM.year===new Date().getFullYear() && histYM.month===new Date().getMonth()+1)
...
function attachDelHandlers(list) {
...
function renderChronic(ref) {
  // Use 30 days for chronic analysis (longer window = more useful)
  const cutoff=new Date(); cutoff.setDate(cutoff.getDate()-30);
  const recent=records.filter(r=>r.date>=toDS(cutoff));
  const cb=$("chronic-body");
  if(recent.length<3){ cb.innerHTML=`<div class="ch-ok">📊 3食以上記録すると慢性的な栄養傾向を分析できます</div>`; return; }
  ...
  const FOODS={protein:["鶏むね肉","卵","納豆","豆腐","ツナ缶","魚介類"],fat:["ナッツ","アボカド","さば","チーズ","オリーブオイル"],carbs:["ご飯","パン","パスタ","バナナ","さつまいも"],fiber:["ブロッコリー","海藻","きのこ","オートミール","さつまいも"],calcium:["牛乳","ヨーグルト","豆腐","小魚","チーズ"],iron:["レバー","ほうれん草","枝豆","あさり","赤身肉"],vitaminC:["ブロッコリー","パプリカ","いちご","みかん","キウイ"],B1:["豚肉","豆類","全粒粉","ナッツ"],B2:["乳製品","卵","レバー","納豆"],B6:["肉類","魚","バナナ","大豆"]};
  ...
  }).filter(a=>a.avg<80).sort((a,b)=>a.avg-b.avg);

  if(!analysis.length){ cb.innerHTML=`<div class="ch-ok">🎉 直近30日間の栄養バランスは概ね良好です！</div>`; return; }

  // Header note
  const noteHtml=`<p style="font-size:.72rem;color:var(--text3);margin-bottom:.8rem">直近${dates.length}日・${recent.length}食の記録から分析</p>`;

  cb.innerHTML=noteHtml+analysis.map(({k,avg})=>{
